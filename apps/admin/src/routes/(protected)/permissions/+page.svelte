<script lang="ts">
  ...
  import { AlertDialog, Badge, Button, DataTable, Table } from "@hms/ui";
  ...
  const rows = $derived(($permissionsQuery.data as { data: Permission[] } | undefined)?.data ?? []);
  ...
  <div class="flex items-center justify-between">
    <div>
      ...
      <p class="mt-1 text-sm text-muted-foreground">
        Define which modules are available across the platform. Organizations are assigned from this
        catalog.
      </p>
    </div>
  ...
  <AlertDialog.Content>
    <AlertDialog.Header>
      ...
      <AlertDialog.Description>
        <strong>{deactivateTarget?.name}</strong> will be hidden from all organization assignment pages.
        Existing assignments are preserved — this is a soft delete and can be reversed.
      </AlertDialog.Description>
    </AlertDialog.Header>
    {#if deactivateError}
