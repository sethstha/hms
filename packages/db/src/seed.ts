...
async function seed() {
  ...
  for (const perm of BUILT_IN_PERMISSIONS) {
    await db
      .insert(permissions)
      .values({ ...perm, isActive: true })
      .onConflictDoNothing();
  }

  // 3. Grant all permissions to demo org
  console.log("🔑 Granting all permissions to demo org...");
  const superadminId = "10000000-0000-0000-0000-000000000001";
  for (const perm of BUILT_IN_PERMISSIONS) {
    await db
      .insert(organizationPermissions)
      .values({ organizationId: DEMO_ORG.id, permissionId: perm.id, grantedBy: superadminId })
      .onConflictDoNothing();
  }

  // 4. Users + accounts + memberships
  console.log("\n👥 Creating demo users...");
